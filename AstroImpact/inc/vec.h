...
static inline float randfn(); // box-muller normal [bi-directional]
static inline int vec_ftoi(float f); // float to integer quantise

// normalising the result is optional / at the callers responsibility
static inline void vRuv(vec* v);   // Random Unit Vector
static inline void vRuvN(vec* v);  // Normal Random Unit Vector
static inline void vRuvBT(vec* v); // Brian Tung Random Unit Vector (on surface of unit sphere)
static inline void vRuvTA(vec* v); // T.Davison Trial & Error (inside unit sphere)
static inline void vRuvTD(vec* v); // T.Davison Random Unit Vector Sphere

static inline void  vCross(vec* r, const vec v1, const vec v2);
static inline float vDot(const vec v1, const vec v2);
static inline float vSum(const vec v);
static inline void  vReflect(vec* r, const vec v, const vec n);

static inline int  vEqualTol(const vec a, const vec b, const float tol);
static inline int  vEqualInt(const vec a, const vec b);
static inline void vMin(vec* r, const vec v1, const vec v2);
static inline void vMax(vec* r, const vec v1, const vec v2);

static inline void  vNorm(vec* v);
static inline float vDist(const vec v1, const vec v2);
static inline float vDistSq(const vec a, const vec b);
static inline float vDistMh(const vec a, const vec b); // manhattan
static inline float vDistLa(const vec a, const vec b); // longest axis
static inline float vMod(const vec v); // modulus
static inline float vMag(const vec v); // magnitude
static inline void  vInv(vec* v); // invert
static inline void  vCopy(vec* r, const vec v);
static inline void  vDir(vec* r, const vec v1, const vec v2); // direction vector from v1 to v2

static inline void vRotX(vec* v, const float radians);
static inline void vRotY(vec* v, const float radians);
static inline void vRotZ(vec* v, const float radians);

static inline void vAdd(vec* r, const vec v1, const vec v2);
static inline void vSub(vec* r, const vec v1, const vec v2);
static inline void vDiv(vec* r, const vec numerator, const vec denominator);
static inline void vMul(vec* r, const vec v1, const vec v2);

static inline void vAddS(vec* r, const vec v1, const float v2);
static inline void vSubS(vec* r, const vec v1, const float v2);
static inline void vDivS(vec* r, const vec v1, const float v2);
static inline void vMulS(vec* r, const vec v1, const float v2);
...
static inline void vRuvN(vec* v)
{
    v->x = randfn();
    v->y = randfn();
    v->z = randfn();
...
static inline void vRuvBT(vec* v)
{
...
static inline void vRuvTA(vec* v)
{
...
static inline void vRuvTD(vec* v)
{
    // T.P.Davison@tees.ac.uk
...
static inline float vDistMh(const vec a, const vec b)
{
    return (a.x - b.x) + (a.y - b.y) + (a.z - b.z);
}

static inline float vDistLa(const vec v1, const vec v2)
{
...
static inline int vec_ftoi(float f)
{
    if(f < 0.f)
        f -= 0.5f;
    else
...
static inline float vMod(const vec v)
{
    return sqrtps(v.x*v.x + v.y*v.y + v.z*v.z);
}

static inline float vMag(const vec v)
{
...
static inline void vDir(vec* r, const vec v1, const vec v2)
{
    vSub(r, v2, v1);
    vNorm(r);
}
